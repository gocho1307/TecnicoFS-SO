...
int link_(size_t rep_target, size_t file_idx_target, size_t rep_name,
...
int main() {
    ...
    for (size_t rep = 0; rep < FILE_COUNT; rep++) {
        ...
        {
            int fd = open_(rep, rep, TFS_O_CREAT);
            assert(fd != -1);
        ...
        for (size_t i = 0; i < FILE_COUNT; i++) {
            ...
            if (i == 0) {
                ...
                assert(link_(rep, rep, rep, i) != -1);
            } else {
                // link to the link created in the previous iteration
                assert(link_(rep, i - 1, rep, i) != -1);
            }
        }
